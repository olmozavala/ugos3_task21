...
# Example:
# % ./subset_dataset_oisst.py -s 20100101 -f 20100201 -b -140 -110 20 30 -x MUR-JPL-L4-GLOB-v4.1
...
def parseoptions():
  ...
  # parser.add_option("-o", "--output_folder", help="output folder to save", action="store_true", dest="output_folder",default=".")
  ...
  while (bmore > 0):
   ...
   for arrays in doc.getElementsByTagName('link'):
    ...
    if names == 'OPeNDAP URL':
      ...
      if status_curl == 0:
        # cmd=F"curl -g '{cmd}' -o {join(str(options.output_folder),str(ncout))}"
        cmd='curl -g "'+cmd+'" -o '+ ncout
      elif status_wget == 0:
        # cmd=F"wget '{cmd}' -O {join(str(options.output_folder),str(ncout))}"
